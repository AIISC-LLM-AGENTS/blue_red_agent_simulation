...
class Red_Agent:
  ...
  def red_meets_noone(self, iteration):
    ...
    if(self.previous_CoT == ""):
      prompt = red_meets_noone(self.original_source, self.original_destination, self.current_place)
    else:
      prompt = red_meets_noone(self.original_source, self.original_destination, self.current_place)
  ...
  def blue_agent_meets_red_agent(self, blue_agent, iteration):
    ...
    """
    print("Hit")
    prompt = red_meets_blue(blue_agent.original_source, blue_agent.original_destination, blue_agent.current_place, self.previous_CoT, self.original_source, self.original_destination, self.current_place)
    next_place = ""
    CoT = ""
  ...
  def red_agent_meets_red_agent_first_agent(self, red_agent_1, red_agent_2, iteration):
    ...
    """
    print("Hit")
    prompt = red_meets_red(red_agent_1.original_source, red_agent_1.original_destination, red_agent_1.current_place, red_agent_1.previous_CoT, red_agent_2.original_source, red_agent_2.original_destination, red_agent_2.current_place, "")
    next_place = ""
    CoT = ""
  ...
  def red_agent_meets_red_agent_second_agent(self, red_agent_1, red_agent_2, iteration):
      ...
      """
      print("Hit")
      prompt = red_meets_red(red_agent_2.original_source, red_agent_2.original_destination, red_agent_2.current_place, red_agent_2.previous_CoT, red_agent_1.original_source, red_agent_1.original_destination, red_agent_1.current_place, metadata.RED_RESPONSE_TO_RED[-1])
      next_place = ""
      CoT = ""
      red_response_to_red = "None"
